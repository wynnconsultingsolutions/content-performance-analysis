...
-- Median, for the same reason as query 1. The mean column is kept alongside
-- it to show the size of the distortion: under $5M it reads 45.9x against a
-- 2.50x median, which inverts the pattern entirely.
...
ranked AS (
    SELECT
        budget_tier,
        return_multiple,
        ROW_NUMBER() OVER (PARTITION BY budget_tier
                           ORDER BY return_multiple)     AS rn,
        COUNT(*)     OVER (PARTITION BY budget_tier)     AS n
    FROM tiered
),
median_return AS (
    SELECT budget_tier, AVG(return_multiple) AS median_return
    FROM ranked
    WHERE rn IN ((n + 1) / 2, (n + 2) / 2)
...
SELECT
    ...
    COUNT(*)                                            AS titles,
    ROUND(m.median_return, 2)                           AS median_return_x,
    ROUND(AVG(t.return_multiple), 1)                    AS mean_return_x,
    ROUND(100.0 * AVG(t.profitable), 1)                 AS pct_profitable,
    ROUND(AVG(t.worldwide_revenue) / 1e6, 1)            AS avg_revenue_musd,
    ROUND(AVG(t.international_share) * 100, 1)          AS avg_intl_share_pct
FROM tiered t
JOIN median_return m ON m.budget_tier = t.budget_tier
GROUP BY t.budget_tier, m.median_return
ORDER BY t.budget_tier;
